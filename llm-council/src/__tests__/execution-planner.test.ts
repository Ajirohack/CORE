...
// Define ProcessingPhase enum to match what's used in execution-planner.ts
...
// Now that ExecutionPlanner has been fixed, we can run the tests
describe("ExecutionPlanner", () => {
  ...
  beforeEach(() => {
    ...
    mockSharedContext = {
      prepareSpecialistInput: jest
        .fn()
        .mockReturnValue({ query: "test query" }),
  ...
  it("should create a basic execution plan", async () => {
    ...
    const plan = await executionPlanner.createPlan(
      request,
      mockSharedContext as unknown as SharedContext
    );
  ...
  it("should handle dependencies correctly", async () => {
    ...
    const stepsWithDependencies = plan.steps.filter(
